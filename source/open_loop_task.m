...
%% Settings: Paths
CL = struct;
CL.now = datestr(now, 'dd-mm-yyyy HH:MM:SS');
...
CL.stim_onset = nan(n_trials, 1);
CL.n_target_frames = cfg.n_target_frames;
CL.prestim_frames = randi([cfg.n_min_prestim_frames, ...
    cfg.n_max_prestim_frames], n_trials, 1);
CL.n_delay_frames = randi([cfg.n_min_delay_frames, ...
    cfg.n_max_delay_frames], n_trials, 1);
CL.RT = zeros(size(CL.stim_present));
...
% Extract the Perlin noise from the file (keep only color channel since it
% is gray-scale) and discard other data.
...
while trial <= n_trials
    ...
    for i = 1:CL.prestim_frames(trial)
    ...
    for i = 1:CL.n_target_frames
        ...
        % Update the screen.
        %
        [vbl, target_onset_time] = Screen('Flip', window_ptr, ...
            vbl + 0.5 * cfg.ifi);
        CL.stim_onset(trial) = target_onset_time;
        Screen('Close', noise_texture);
    ...
    % Delay
    for i = 1:CL.n_delay_frames(trial)
        % Draw the Perlin noise
        [noise_texture, noise_interp_step, noise_frame_ind] = ...
            update_noise(window_ptr, perlin_noise, noise_frame_ind, ...
            noise_interp_step, cfg.n_noise_interp_steps);             
    ...
    while 1
        ...
        [key_down, answer_time, key_code] = KbCheck;
        CL.RT(trial) = answer_time - target_onset_time;        
        if (key_down)
